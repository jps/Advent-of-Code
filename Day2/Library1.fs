...
type Day2() = 
    ...
    static member LoadDimensionsFromFile(filePath) = 
        let linesList = (System.IO.File.ReadLines(filePath)) |> List.ofSeq        
        let dimensions = List.map (fun b -> 
            let split = b.ToString().Split[|'x'|]
            let splitConverted = Array.map(fun str -> Int32.Parse(str)) split
            {   Length = (splitConverted.[0]);
                Width = (splitConverted.[1]);
                Height = (splitConverted.[2]) }) linesList
        dimensions
